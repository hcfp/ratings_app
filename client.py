...
def list_all(host):
    response = s.get(f"http://{host}:8000/ratings/list").json()["list"]
...
def view(host):
    response = s.get(f"http://{host}:8000/ratings/view").json()["view"]
    for item in response:
...
def average(host, prof_code, module_code):
    ...
    response = s.post(f"http://{host}:8000/ratings/average", data=data).json()
...
def rate(host, prof_code, module_code, year, semester, score):
    data = {"professor-code": prof_code,
            ...
            "semester": semester,
            "score": score}
    response = s.post(f"http://{host}:8000/ratings/rate", data=data).json()
